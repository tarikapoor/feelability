...
import type { Profile, Review, ReviewRelationship } from "@/app/types";
...
const relationshipOptions: { value: ReviewRelationship; label: string }[] = [
  { value: "same_team", label: "Worked in the same team" },
  { value: "friend", label: "As a Friend" },
  { value: "manager", label: "I am her/his Manager" },
  { value: "collaborator", label: "I collaborated with her/him on projects" },
];
...
export default function MirrorProfilePage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!relationship) {
      setErrorMessage("Please tell us how you know this person.");
      return;
    }
    if (!rating) {
          ...
          <section ref={formRef} className="bg-white/80 border border-gray-100 rounded-2xl p-6 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative md:max-w-sm" ref={relationshipRef}>
                  <button
                    type="button"
                    onClick={() => setRelationshipOpen((open) => !open)}
                    aria-haspopup="listbox"
                    aria-expanded={relationshipOpen}
                    className="flex w-full items-center justify-between gap-2 rounded-lg border border-gray-200 bg-white pl-3 pr-3 py-2 text-sm text-left text-gray-700"
                  >
                    <span className={relationship ? "text-gray-700" : "text-gray-400"}>
                      {relationship
                    ...
                    >
                      {relationshipOptions.map((option) => (
                        <button
                          ...
                          type="button"
                          role="option"
                          aria-selected={relationship === option.value}
                          onClick={() => {
              ...
              {filteredGroups.map((group) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {group.relationship && (
                      <span className="text-xs font-medium text-purple-700 bg-purple-50 border border-purple-100 rounded-full px-2.5 py-1">
